...
app.get("/pay", async function (req, res) {
  ...
  axios
    ...
    .then(function (response) {
      ...
      res.send(response.data); // Send response data instead of response object
    })
    .catch(function (error) {
      console.error("Payment initiation error:", error.message);
      res
        .status(500)
        .send({ msg: "Payment initiation failed", error: error.message });
    });
});
  ...
  if (merchantTransactionId) {
    ...
    axios
      ...
      .catch(function (error) {
        ...
        res
          .status(500)
          .send({ msg: "Error checking payment status", error: error.message });
      });
  } else {
    res.status(400).send("Missing merchant transaction ID");
  }
});
